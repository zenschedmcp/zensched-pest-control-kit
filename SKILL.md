...
6. **Access notes and applicator license numbers stay local.** `properties.access_notes` (gate codes, crawl hatches, dogs, alarm words) and `technicians.license_no` must **never** be sent to ZenSched: not in `location_create` `notes`, not in `event_create` `notes` or `title`, not in a form, not in a `shift_cancel` reason. Tell the tech these in person or by a channel the owner chooses. If the owner asks you to put a code or license number in ZenSched, decline and explain why.
